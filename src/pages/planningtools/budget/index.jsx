...
import { Layout, Card, Col, Row, List, Input, Modal, Button, Divider } from 'antd';
import { PlusCircleOutlined, EllipsisOutlined, CalculatorOutlined } from '@ant-design/icons';
...
export const Budget = () => {
    ...
    return (
        <Layout>
                            ...
                            <Col span={8} className={clsx(styles.col_line)}>
                                <Card bordered={false} className={clsx(styles.detail)}>
                                    <div className={clsx(styles.add_category_group)}>
                                        <PlusCircleOutlined className={clsx(styles.add_category)} onClick={showModalAdd} />
                                        <p className={clsx(styles.add_text)} onClick={showModalAdd} >Add new category</p>
                                    </div>
                                    ...
                                    <List size="small" className={clsx(styles.list_category)}>
                                        <VirtualList
                                            ...
                                            onScroll={onScroll}
                                        >
                                            {item => <List.Item className={clsx(styles.list_content)}>
                                                <Card className={clsx(styles.list_item)}>
                                                    {item.type} <span>$ {item.value} <Popup 
                                                        trigger={<EllipsisOutlined className={clsx(styles.edit_dot)} />}
                                                    ...
                                                    >
                                                        <div className={clsx(styles.edit_button)}>
                                                            ...
                                                            <a onClick={() => {showModalEdit(); getItem(item)}}>Edit</a><br/>
                                                            <Divider className={clsx(styles.divider_edit)}/>
                            ...
                            <Col span={15} className={clsx(styles.col_line)}>
                                <Card bordered={false} className={clsx(styles.detail)}>
                                    <div className={clsx(styles.budget_right_title)}>
                                        <CalculatorOutlined className={clsx(styles.estimating_icon)} />
                                        <p className={clsx(styles.chart_text)} >Estimating cost: <t />
                                            <Input
                                                className={clsx(styles.estimate_budget)}
                                                value={"$ "+ sum}
                                                disabled />
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <Input
                                            ...
                                            placeholder="Input your target budget" type="number"
                                            prefix={"$"}
                                            onChange={() => setSave(true)} />

                                        <Button className={clsx(styles.save_btn, save ? styles.input_change_save_btn : null)}
                                            onClick={() => setSave(false)}>Save</Button>
                                    </div>
                                    <div className={clsx(styles.chart_content)}>
                                        ...
                                        <p className={clsx(styles.chart_text)}>Expenses tendency radar chart</p>
                                    </div>
                                </Card>
                            </Col>
                        </Row>
            ...
            <Modal className={clsx(styles.modal_add_category)} title="Add Category" visible={isModalVisible_add} onCancel={handleCancelAdd} onOk={handleAdd} okText="Add" >
                <Input className={clsx(styles.input_name_category)}
                    placeholder="Input category name" type="text" value={name}
                    onChange={(e) => setName(e.target.value)} />
                <Input className={clsx(styles.input_cost_category)}
                    placeholder="Input Category Cost" type="number" value={cost}
                    onChange={(e) => setCost(e.target.value)} />
                    
            </Modal>

            <Modal className={clsx(styles.modal_edit_category)} title="Edit Category" visible={isModalVisible_edit} onCancel={handleCancelEdit} onOk={handleEdit} okText="Save" >
                <Input className={clsx(styles.input_name_category)}
                    placeholder="Edit Category Name" type="text" value={name}
                    onChange={(e) => setName(e.target.value)} />
                <Input className={clsx(styles.input_cost_category)}
                    placeholder="Edit Category Cost" type="number" value={cost}
                    onChange={(e) => setCost(e.target.value)} />
            </Modal>
            <Footer />
        </Layout>
